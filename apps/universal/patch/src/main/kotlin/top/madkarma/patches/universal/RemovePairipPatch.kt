...
private fun noOp(classDef: DexClass, vararg methodNames: String): Int {
    var patched = 0
...
private fun returnTrue(classDef: DexClass, vararg methodNames: String): Int {
    var patched = 0
    for (methodName in methodNames) {
        classDef.methods.filter { it.name == methodName }.forEach {
    ...
    for (classDef in scope.classes) {
        ...
        for (method in classDef.methods) {
            ...
            when (method.returnType) {
                ...
                "Ljava/lang/String;" -> if (!stringDone) {
...
val removePairip =
    patch("Remove Pairip") {
        description(
            "Removes Pairip (Google Play automatic integrity protection): no-ops the license " +
                "client, signature check, license activity, and content provider, forces the " +
                "license response to success, disables repeated background checks, removes the " +
                "Pairip Application wrapper, and strips injected com.pairip.* manifest " +
                "components. Optionally guts the Pairip VM. Does not bypass server-side Play " +
                "Integrity attestation or pairipcore virtualization.",
        )

        val gutVmRunner = boolOption(
            "gutVmRunner",
            title = "Gut Pairip VM",
            description = "Also neutralizes the Pairip VM: makes VMRunner.invoke() return null, " +
                "drops the pairipcore native library load, and no-ops StartupLauncher.launch(). " +
                "Only enable this for apps whose VM merely runs the startup integrity/license " +
                "program (its result is discarded). If the app routes real functionality through " +
                "the VM (e.g. broadcast receivers or IAP), enabling this breaks those features. " +
                "Leave off unless the app needs it.",
            default = false,
        )
        ...
        val spoofInstaller = boolOption(
            ...
            description = "Rewrites leftover installer-origin checks that reference the Play " +
        ...
        execute {
            ...
            manifest.edit {
                findByTag("application").firstOrNull()?.let { application ->
                    if (application["android:name"] == PAIRIP_APPLICATION) {
                        bytecode.findClass(PAIRIP_APPLICATION)?.superclass
            ...
            if (patched == 0) {
                log.info("Pairip: no Pairip methods found; no bytecode changes applied.")
            } else {
